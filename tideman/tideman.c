...
void record_preferences(int ranks[])
{
    ...
    for (int rank = 0; rank < candidate_count; rank++)
    {
        ...
        {
            preferences[ranks[rank]][ranks[column]] += 1;
        }
    }
    return;
}
...
void sort_pairs(void)
{
    ...
    for (int i = pair_count - 1; i >= 0; i--)
    {
        ...
        // exchange strongest win with index
...
bool noCycle(int winner, int loser)
{
    ...
    {
        // recursion
        if (locked[loser][i] == true && noCycle(winner, i))
        {
            return true;
        }
    }
